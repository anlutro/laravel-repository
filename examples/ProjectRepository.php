...
class ProjectRepository extends \c\EloquentRepository
{
	 ...
	 */
	public function prepareQuery($query, $many)
	{
	...
	public function prepareCollection($collection)
	{
		...
		$extraData = ExtraData::whereIn($collection->lists('id'))->get();

		// iterate through the collection...
		foreach ($collection as $item) {
			// check if we have extra data and add it.
			if ($extraData->has($item->id)) {
				$item->extra_data = $extraData[$item->id];
			} else {
	 ...
	 */
	public function readyForCreate($project)
	{
